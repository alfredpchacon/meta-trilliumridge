...
typedef struct
{
    float    Pos[NUM_GIMBAL_AXES]; //!< Track box offset from the line of sight in radians
    float    Size;                 //!< Track box size in percent of image width
    float    Confidence;           //!< Tracker confidence, from 0 to 1
    uint32_t Coasting;            
    uint32_t Active;               //!< Set to 1 if the track is active, or 0 if it is currently inactive
}PrimaryTrackData_t;

//! return the minimum encoded length for the PrimaryTrackData_t structure
#define getMinLengthOfPrimaryTrackData_t() (2*NUM_GIMBAL_AXES+3)

//! Encode a PrimaryTrackData_t structure into a byte array
void encodePrimaryTrackData_t(uint8_t* data, int* bytecount, const PrimaryTrackData_t* user);

//! Decode a PrimaryTrackData_t structure from a byte array
...
typedef struct
{
    ...
    unsigned         WatchdogLock : 1;    //!< Set to 1 if the laser was locked due to laser command comm timeout. A new command will clear this lock
    unsigned         Reserved : 13;       //!< Reserved for future use. Field is encoded constant.
...
typedef struct
{
    ...
    float              imageRotation;                
    int8_t             cameraIndex;                   //!< Current camera index, or -1 if the gimbal does not include this field
    uint8_t            hasTrackData;                  //!< Set to 1 if this packet includes object track data, otherwise 0
    PrimaryTrackData_t primaryTrackData;              //!< Primary object track metadata
...
#define getGeolocateTelemetryCoreMaxDataLength() (8*NUM_GIMBAL_AXES+84)
...
typedef struct
{
    ...
    float CurrentLaser;    //!< Current in Amps of the Lsr rail
}OrionDiagnostics_t;
...
#define getOrionDiagnosticsMaxDataLength() (32)
...
{
    FAULT_TYPE_NONE,                     
    FAULT_TYPE_CLEVIS_RESET_COMMANDED,   
    FAULT_TYPE_INVALID_GYRO_CALIBRATION, 
    FAULT_TYPE_VELOCITY_LIMIT_EXCEEDED,  
    FAULT_TYPE_ACCELERATION_LIMIT_EXCEEDED 
} OrionFaultType_t;
...
typedef enum
{
    FAULT_COMPONENT_PAN_AXIS,    
    FAULT_COMPONENT_TILT_AXIS,   
    FAULT_COMPONENT_PAYLOAD_GYROS 
} OrionFaultComponent_t;
...
typedef struct
{
    OrionFaultType_t      Type;      //!< Fault type enum
    OrionFaultLevel_t     Level;     //!< Fault level enum
    OrionFaultComponent_t Component; //!< Fault component enum
    uint32_t              Code;      //!< Fault code is fault type dependent
    float                 Value;     //!< Fault value is fault type dependent
}OrionFault_t;

//! Create the OrionFault packet
void encodeOrionFaultPacketStructure(void* pkt, const OrionFault_t* user);

//! Decode the OrionFault packet
int decodeOrionFaultPacketStructure(const void* pkt, OrionFault_t* user);

//! return the packet ID for the OrionFault packet
#define getOrionFaultPacketID() (ORION_PKT_FAULTS)

//! return the minimum encoded length for the OrionFault packet
#define getOrionFaultMinDataLength() (11)

//! return the maximum encoded length for the OrionFault packet
#define getOrionFaultMaxDataLength() (11)
...
typedef enum
{
    crownModeNormal, //!< The normal operation of the crown board
    crownModeLogINS, //!< Normal mode of operation except INS raw data are sent to users
    crownModePrintIMU,//!< Normal mode of operation except the IMU sensors are printed out using debug packets
    crownModePrintINS,//!< Normal mode of operation except the INS states are printed out using debug packets
    crownModeTempCal,//!< Special mode of operation used for temperature calibration of the sensors
    crownModeIMUCal  //!< Special mode of operation used for gain, bias, and alignment calibration of the IMU
} crownModes;

//! Create the CrownMode packet
void encodeCrownModePacket(void* pkt, crownModes mode);

//! Decode the CrownMode packet
int decodeCrownModePacket(const void* pkt, crownModes* mode);

//! return the packet ID for the CrownMode packet
#define getCrownModePacketID() (ORION_PKT_CROWN_MODE)

//! return the minimum encoded length for the CrownMode packet
#define getCrownModeMinDataLength() (1)

//! return the maximum encoded length for the CrownMode packet
#define getCrownModeMaxDataLength() (1)
...
typedef struct
{
    float    Gyros[3];  //!< Measured angular rate in body axes in radians per second
    float    Accels[3]; //!< Measured specific force in body axes in meters per second per second
    float    Baro;      //!< Measured barometric pressure in Pascals
    float    OAT;       //!< Measured outside air temperature in degrees Kelvin
    uint32_t time;      //!< system time stamp in milliseconds of this IMU data
    float    gyroTemp;  //!< Gyro die temperature in degrees Kelvin
    float    gyroDt;    //!< Elapsed gyro time in seconds. Use this time to compute delta angle from the gyro rate
    float    accelDt;   //!< Elapsed accelerometer time in seconds. Use this time to compute delta velocity from the specific force
    int16_t  ppsAge;    //!< PPS pulse age in milliseconds before the system time reported in this packet
    uint8_t  counter;   //!< IMU packet counter, rolls over every 256
}OrionSensorData_t;

//! Create the OrionSensorData packet
void encodeOrionSensorDataPacketStructure(void* pkt, const OrionSensorData_t* user);

//! Decode the OrionSensorData packet
int decodeOrionSensorDataPacketStructure(const void* pkt, OrionSensorData_t* user);

//! return the packet ID for the OrionSensorData packet
#define getOrionSensorDataPacketID() (ORION_PKT_SENSOR_DATA)

//! return the minimum encoded length for the OrionSensorData packet
#define getOrionSensorDataMinDataLength() (22)

//! return the maximum encoded length for the OrionSensorData packet
#define getOrionSensorDataMaxDataLength() (29)
...
typedef struct
{
    ...
    float       multiAntHeadingAccuracy;   //!< Multi-antenna heading accuracy in radians
}GpsData_t;
...
#define getGpsDataMaxDataLength() (73)
...
typedef struct
{
    float   Zoom;             //!< Zoom is calculated from fov as: Zoom = PixelPitch*ArrayWidth*0.5/(MinFocalLength*tan(FOV*0.5)) array width and pixel pitch come from the OrionCameras packet Sending 0 or less commands current zoom, 1 is full wide.
    float   Focus;            //!< Focus command/position on imagers with manual focus control, expected between 0 and 1.0, exactly 0 commands autofocus, -1 is don't change
    uint8_t KeepActiveCamera; //!< Set to 1 to maintain the currently active camera. If this bit is set to zero and the Index field does not match the currently active camera index, the gimbal will switch cameras
...
typedef struct
{
    ...
    unsigned      Palette : 3;      //!< Image Colorization - 2 for Rainbow, 3 for RainbowHC, 4 for Ironbow, 5 for Lava, 6 for Arctic, 7 for Glowbow, 8 for GradedFire, 9 for Hottest
    ...
    float         AgcGamma;         //!< Agc Gamma (default = 1.0, effective min = 0.5, effective max = 4.0)
    float         AgcLinearPercent; //!< Agc Linear Percent (default = 50.0, effective min = 0.0, effective max = 100.0)
}OrionFlirSettings_t;
...
#define getOrionFlirSettingsMaxDataLength() (10)
...
typedef struct
{
    uint8_t Index;           //!< Index of the camera that these settings should apply to. Use 0xFF to apply to any connected Attollo camera.
    float   MinGain;         //!< Minimum AGC gain
    float   MaxGain;         //!< Maximum AGC gain
    float   MinExposure;     //!< Minimum AEC integration time
    float   MaxExposure;     //!< Maximum AEC integration time
    ...
    uint8_t MeanTargetRange; //!< valid(0-5) corisponding to 0-2000, ... , 10000-12000
    uint8_t BlackPoint;      //!< Sets Minimum Offset Range(x1000)
}OrionAttolloSettings_t;

//! Create the OrionAttolloSettings packet
void encodeOrionAttolloSettingsPacketStructure(void* pkt, const OrionAttolloSettings_t* user);

//! Decode the OrionAttolloSettings packet
int decodeOrionAttolloSettingsPacketStructure(const void* pkt, OrionAttolloSettings_t* user);

//! return the packet ID for the OrionAttolloSettings packet
#define getOrionAttolloSettingsPacketID() (ORION_PKT_ATTOLLO_SETTINGS)

//! return the minimum encoded length for the OrionAttolloSettings packet
#define getOrionAttolloSettingsMinDataLength() (9)

//! return the maximum encoded length for the OrionAttolloSettings packet
#define getOrionAttolloSettingsMaxDataLength() (13)

typedef enum
{
    COORD_FMT_DEG_DECIMAL,//!< Decimal degrees
    COORD_FMT_DEG_MIN,   //!< Degrees + decimal minutes
    COORD_FMT_DEG_MIN_SEC,//!< Degrees + minutes + seconds
    COORD_FMT_MGRS,      //!< MGRS
    COORD_FMT_UTM        //!< UTM
} CoordFormat_t;
...
typedef struct
{
    ...
    uint32_t      MetricAlt;     
    uint32_t      MetricDist;    
    CoordFormat_t CoordFormat;   
    uint32_t      Date;          
    uint32_t      Time;          
    uint32_t      NorthIndicator;
    char          UserString[40];
}TelemetryOptions_t;
...
typedef struct
{
    uint32_t           Stabilized;          //!< Set to 1 to enable digital video stabilization, or 0 to disable
    uint32_t           ShowTrackBox;        //!< Set to 1 to render the track box on the video, or 0 to only send KLV track box position
    uint32_t           ShowReticle;         //!< Set to 1 to render the center reticle on the video
    uint32_t           RotateVideo;         //!< Set to 1 to automatically rotate the video to compensate for camera roll angles.
    uint32_t           DisableFovMatch;     //!< Set to 1 to disable FOV matching
    uint32_t           ShowTelemetry;       //!< Set to 1 to render telemetry overlays on the video
    uint32_t           HasTelemetryOptions; //!< Set to 1 if TelemetryOptions are included
    uint32_t           ShowDetections;      //!< Set to 1 to render MTI detections, if applicable
    uint32_t           DisableNtsc;         //!< Set to 1 to disable NTSC analog video, or 0 to enable
    uint32_t           EnablePip;           //!< Set to 1 to enable picture-in-picture overlays, or 0 to disable
    uint32_t           ZoomTrackBox;        //!< Set to 1 to enable 1.5x trackbox zooming, or 0 to disable
    float              MaxShift;            //!< Maximum stabilization shift as a percentage of frame size, or 0 for no change
    TelemetryOptions_t TelemetryOptions;   
}VideoOptions_t;

//! Create the VideoOptions packet
void encodeVideoOptionsPacketStructure(void* pkt, const VideoOptions_t* user);

//! Decode the VideoOptions packet
int decodeVideoOptionsPacketStructure(const void* pkt, VideoOptions_t* user);

//! return the packet ID for the VideoOptions packet
#define getVideoOptionsPacketID() (ORION_PKT_VIDEO_OPTIONS)

//! return the minimum encoded length for the VideoOptions packet
#define getVideoOptionsMinDataLength() (4)

//! return the maximum encoded length for the VideoOptions packet
#define getVideoOptionsMaxDataLength() (49)
...
typedef enum
{
    ...
    PROTOCOL_UBLOX_HDG,      //!< uBlox relative heading
    PROTOCOL_SBG_PULSE_IMU,  //!< SBG Pulse IMU
    NUM_PROTOCOL             
} OrionProtocols_t;
...
typedef struct
{
    ...
    uint8_t  MaxTcpClients;    //!< Maximum number of TCP telemetry clients; set to zero for 'no change'
...
#define getOrionNetworkSettingsMaxDataLength() (19)
...
#define getOrionNetworkByteSettingsMaxDataLength() (19)
...
typedef struct
{
    ...
    uint32_t OnTime;         //!< Board on time in minutes
    int8_t   HwType;         //!< Hardware type identifier, or -1 if unknown
    int8_t   HwRevision;     //!< Hardware revision identifier, or -1 if unknown
...
#define getOrionPayloadVersionMaxDataLength() (46)
...
typedef struct
{
    unsigned HasIAS : 1;   //!< Set to 1 if this message includes IAS, otherwise set to zero
    unsigned HasTAS : 1;   //!< Set to 1 if this message includes TAS, otherwise set to zero
    unsigned IsFlying : 1; //!< Set to 1 if flying, otherwise 0
    ...
    float    IAS;          //!< Indicated airspeed (IAS), in m/s
    float    TAS;          //!< True airspeed (TAS), in m/s
...
#define getOrionAutopilotDataMaxDataLength() (6)
