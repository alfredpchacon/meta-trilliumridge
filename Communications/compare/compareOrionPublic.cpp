...
QString compareOrionFlirSettings_t(QString prename, const OrionFlirSettings_t* user1, const OrionFlirSettings_t* user2)
{
    ...
    if(user1->AgcGamma != user2->AgcGamma)
        report += prename + ":AgcGamma"  + " '" + QString::number(user1->AgcGamma, 'g', 16) + "' '" + QString::number(user2->AgcGamma, 'g', 16) + "'\n";

    if(user1->AgcLinearPercent != user2->AgcLinearPercent)
        report += prename + ":AgcLinearPercent"  + " '" + QString::number(user1->AgcLinearPercent, 'g', 16) + "' '" + QString::number(user2->AgcLinearPercent, 'g', 16) + "'\n";
...
/*!
 * Compare two OrionAttolloSettings_t structures and generate a report of any differences.
 * \param prename is prepended to the name of the data field in the comparison report
 * \param user1 is the first data to compare
 * \param user1 is the second data to compare
 * \return a string describing any differences between user1 and user2. The string will be empty if there are no differences
...
QString compareOrionAttolloSettings_t(QString prename, const OrionAttolloSettings_t* user1, const OrionAttolloSettings_t* user2)
{
    ...
    if(user1->MeanTargetRange != user2->MeanTargetRange)
        report += prename + ":MeanTargetRange"  + " '" + QString::number(user1->MeanTargetRange) + "' '" + QString::number(user2->MeanTargetRange) + "'\n";

    if(user1->BlackPoint != user2->BlackPoint)
        report += prename + ":BlackPoint"  + " '" + QString::number(user1->BlackPoint) + "' '" + QString::number(user2->BlackPoint) + "'\n";

    return report;

}// compareOrionAttolloSettings_t

/*!
 * Compare two OrionAttolloSettings packets and generate a report of any differences.
 * \param prename is prepended to the name of the data field in the comparison report
 * \param pkt1 is the first data to compare
 * \param pkt2 is the second data to compare
 * \return a string describing any differences between pk1 and pkt2. The string will be empty if there are no differences
...
QString compareOrionAttolloSettingsPacket(const void* pkt1, const void* pkt2)
{
    ...
    // Structures to decode into
    OrionAttolloSettings_t struct1, struct2;
    ...
    // Decode each packet
    if(!decodeOrionAttolloSettingsPacketStructure(pkt1, &struct1) || !decodeOrionAttolloSettingsPacketStructure(pkt2, &struct2))
    {
        report = "OrionAttolloSettings packets failed to decode\n";
        return report;
    }

    // Check packet sizes. Even if sizes are different the packets may contain the same result
    if(getOrionPublicPacketSize(pkt1) != getOrionPublicPacketSize(pkt2))
        report += "OrionAttolloSettings packet sizes are different\n";

    report += compareOrionAttolloSettings_t("name", &struct1, &struct2);

    return report;

}// compareOrionAttolloSettingsPacket

//! Compare two TelemetryOptions_t structures and generate a report
QString compareTelemetryOptions_t(QString prename, const TelemetryOptions_t* user1, const TelemetryOptions_t* user2);

/*!
 * Compare two TelemetryOptions_t structures and generate a report of any differences.
 * \param prename is prepended to the name of the data field in the comparison report
 * \param user1 is the first data to compare
 * \param user1 is the second data to compare
 * \return a string describing any differences between user1 and user2. The string will be empty if there are no differences
...
QString compareTelemetryOptions_t(QString prename, const TelemetryOptions_t* user1, const TelemetryOptions_t* user2)
{
    ...
    if(user1->CameraPos != user2->CameraPos)
        report += prename + ":CameraPos"  + " '" + QString::number(user1->CameraPos) + "' '" + QString::number(user2->CameraPos) + "'\n";

    if(user1->TargetPos != user2->TargetPos)
        report += prename + ":TargetPos"  + " '" + QString::number(user1->TargetPos) + "' '" + QString::number(user2->TargetPos) + "'\n";

    if(user1->Fov != user2->Fov)
        report += prename + ":Fov"  + " '" + QString::number(user1->Fov) + "' '" + QString::number(user2->Fov) + "'\n";

    if(user1->Range != user2->Range)
        report += prename + ":Range"  + " '" + QString::number(user1->Range) + "' '" + QString::number(user2->Range) + "'\n";

    if(user1->MetricAlt != user2->MetricAlt)
        report += prename + ":MetricAlt"  + " '" + QString::number(user1->MetricAlt) + "' '" + QString::number(user2->MetricAlt) + "'\n";

    if(user1->MetricDist != user2->MetricDist)
        report += prename + ":MetricDist"  + " '" + QString::number(user1->MetricDist) + "' '" + QString::number(user2->MetricDist) + "'\n";

    if(user1->CoordFormat != user2->CoordFormat)
        report += prename + ":CoordFormat"  + " '" + QString::number(user1->CoordFormat) + "' '" + QString::number(user2->CoordFormat) + "'\n";

    if(user1->Date != user2->Date)
        report += prename + ":Date"  + " '" + QString::number(user1->Date) + "' '" + QString::number(user2->Date) + "'\n";

    if(user1->Time != user2->Time)
        report += prename + ":Time"  + " '" + QString::number(user1->Time) + "' '" + QString::number(user2->Time) + "'\n";

    if(user1->NorthIndicator != user2->NorthIndicator)
        report += prename + ":NorthIndicator"  + " '" + QString::number(user1->NorthIndicator) + "' '" + QString::number(user2->NorthIndicator) + "'\n";

    if(QString::compare(user1->UserString, user2->UserString) != 0)
        report += prename + ":UserString strings differ\n";

    return report;

}// compareTelemetryOptions_t

/*!
 * Compare two VideoOptions_t structures and generate a report of any differences.
 * \param prename is prepended to the name of the data field in the comparison report
 * \param user1 is the first data to compare
 * \param user1 is the second data to compare
 * \return a string describing any differences between user1 and user2. The string will be empty if there are no differences
...
QString compareVideoOptions_t(QString prename, const VideoOptions_t* user1, const VideoOptions_t* user2)
{
    ...
    if(user1->ShowTrackBox != user2->ShowTrackBox)
        report += prename + ":ShowTrackBox"  + " '" + QString::number(user1->ShowTrackBox) + "' '" + QString::number(user2->ShowTrackBox) + "'\n";

    if(user1->ShowReticle != user2->ShowReticle)
        report += prename + ":ShowReticle"  + " '" + QString::number(user1->ShowReticle) + "' '" + QString::number(user2->ShowReticle) + "'\n";

    if(user1->RotateVideo != user2->RotateVideo)
        report += prename + ":RotateVideo"  + " '" + QString::number(user1->RotateVideo) + "' '" + QString::number(user2->RotateVideo) + "'\n";

    if(user1->DisableFovMatch != user2->DisableFovMatch)
        report += prename + ":DisableFovMatch"  + " '" + QString::number(user1->DisableFovMatch) + "' '" + QString::number(user2->DisableFovMatch) + "'\n";

    if(user1->ShowTelemetry != user2->ShowTelemetry)
        report += prename + ":ShowTelemetry"  + " '" + QString::number(user1->ShowTelemetry) + "' '" + QString::number(user2->ShowTelemetry) + "'\n";

    if(user1->HasTelemetryOptions != user2->HasTelemetryOptions)
        report += prename + ":HasTelemetryOptions"  + " '" + QString::number(user1->HasTelemetryOptions) + "' '" + QString::number(user2->HasTelemetryOptions) + "'\n";

    if(user1->ShowDetections != user2->ShowDetections)
        report += prename + ":ShowDetections"  + " '" + QString::number(user1->ShowDetections) + "' '" + QString::number(user2->ShowDetections) + "'\n";

    if(user1->DisableNtsc != user2->DisableNtsc)
        report += prename + ":DisableNtsc"  + " '" + QString::number(user1->DisableNtsc) + "' '" + QString::number(user2->DisableNtsc) + "'\n";

    if(user1->EnablePip != user2->EnablePip)
        report += prename + ":EnablePip"  + " '" + QString::number(user1->EnablePip) + "' '" + QString::number(user2->EnablePip) + "'\n";

    if(user1->ZoomTrackBox != user2->ZoomTrackBox)
        report += prename + ":ZoomTrackBox"  + " '" + QString::number(user1->ZoomTrackBox) + "' '" + QString::number(user2->ZoomTrackBox) + "'\n";

    if(user1->MaxShift != user2->MaxShift)
        report += prename + ":MaxShift"  + " '" + QString::number(user1->MaxShift, 'g', 16) + "' '" + QString::number(user2->MaxShift, 'g', 16) + "'\n";

    report += compareTelemetryOptions_t(prename + ":TelemetryOptions", &user1->TelemetryOptions, &user2->TelemetryOptions);

    return report;

}// compareVideoOptions_t

/*!
 * Compare two VideoOptions packets and generate a report of any differences.
 * \param prename is prepended to the name of the data field in the comparison report
 * \param pkt1 is the first data to compare
 * \param pkt2 is the second data to compare
 * \return a string describing any differences between pk1 and pkt2. The string will be empty if there are no differences
...
QString compareVideoOptionsPacket(const void* pkt1, const void* pkt2)
{
    ...
    // Structures to decode into
    VideoOptions_t struct1, struct2;
    ...
    // Decode each packet
    if(!decodeVideoOptionsPacketStructure(pkt1, &struct1) || !decodeVideoOptionsPacketStructure(pkt2, &struct2))
    {
        report = "VideoOptions packets failed to decode\n";
        return report;
    }

    // Check packet sizes. Even if sizes are different the packets may contain the same result
    if(getOrionPublicPacketSize(pkt1) != getOrionPublicPacketSize(pkt2))
        report += "VideoOptions packet sizes are different\n";

    report += compareVideoOptions_t("name", &struct1, &struct2);

    return report;

}// compareVideoOptionsPacket
...
QString compareOrionNetworkByteSettings_t(QString prename, const OrionNetworkByteSettings_t* user1, const OrionNetworkByteSettings_t* user2)
{
    ...
    if(user1->MaxTcpClients != user2->MaxTcpClients)
        report += prename + ":MaxTcpClients"  + " '" + QString::number(user1->MaxTcpClients) + "' '" + QString::number(user2->MaxTcpClients) + "'\n";
...
QString compareOrionPayloadVersion_t(QString prename, const OrionPayloadVersion_t* user1, const OrionPayloadVersion_t* user2)
{
    ...
    if(user1->HwType != user2->HwType)
        report += prename + ":HwType"  + " '" + QString::number(user1->HwType) + "' '" + QString::number(user2->HwType) + "'\n";

    if(user1->HwRevision != user2->HwRevision)
        report += prename + ":HwRevision"  + " '" + QString::number(user1->HwRevision) + "' '" + QString::number(user2->HwRevision) + "'\n";
