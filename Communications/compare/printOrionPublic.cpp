...
QString textPrintOrionFlirSettings_t(QString prename, const OrionFlirSettings_t* user)
{
    ...
    report += prename + ":AgcGamma"  + " '" + QString::number(user->AgcGamma, 'g', 16) + "'\n";

    report += prename + ":AgcLinearPercent"  + " '" + QString::number(user->AgcLinearPercent, 'g', 16) + "'\n";
...
/*!
 * Generate a string that describes the contents of a OrionAttolloSettings_t structure
 * \param prename is prepended to the name of the data field in the report
 * \param user is the structure to report
 * \return a string containing a report of the contents of user
 */
QString textPrintOrionAttolloSettings_t(QString prename, const OrionAttolloSettings_t* user)
{
    QString report;
    ...
    report += prename + ":MeanTargetRange"  + " '" + QString::number(user->MeanTargetRange) + "'\n";

    report += prename + ":BlackPoint"  + " '" + QString::number(user->BlackPoint) + "'\n";

    return report;

}// textPrintOrionAttolloSettings_t

/*!
 * Generate a string that describes the contents of a OrionAttolloSettings packet
 * \param prename is prepended to the name of the data field in the report
 * \param pkt is the data to print
 * \return a string describing the contents of pkt
 */
QString textPrintOrionAttolloSettingsPacket(const void* pkt)
{
    QString report;

    // Structure to decode into
    OrionAttolloSettings_t user;

    // All zeroes before decoding
    memset(&user, 0, sizeof(user));

    // Decode packet
    if(!decodeOrionAttolloSettingsPacketStructure(pkt, &user))
    {
        report = "OrionAttolloSettings packet failed to decode\n";
        return report;
    }

    // Print the packet size
    report += "OrionAttolloSettings packet size is " + QString::number(getOrionPublicPacketSize(pkt)) + "\n";

    report += textPrintOrionAttolloSettings_t("name", &user);

    return report;

}// textPrintOrionAttolloSettingsPacket

//! Generate a string that describes the contents of a TelemetryOptions_t structure
QString textPrintTelemetryOptions_t(QString prename, const TelemetryOptions_t* user);
...
QString textPrintTelemetryOptions_t(QString prename, const TelemetryOptions_t* user)
{
    ...
    report += prename + ":CameraPos"  + " '" + QString::number(user->CameraPos) + "'\n";

    report += prename + ":TargetPos"  + " '" + QString::number(user->TargetPos) + "'\n";

    report += prename + ":Fov"  + " '" + QString::number(user->Fov) + "'\n";

    report += prename + ":Range"  + " '" + QString::number(user->Range) + "'\n";

    report += prename + ":MetricAlt"  + " '" + QString::number(user->MetricAlt) + "'\n";

    report += prename + ":MetricDist"  + " '" + QString::number(user->MetricDist) + "'\n";

    report += prename + ":CoordFormat"  + " '" + QString::number(user->CoordFormat) + "'\n";

    report += prename + ":Date"  + " '" + QString::number(user->Date) + "'\n";

    report += prename + ":Time"  + " '" + QString::number(user->Time) + "'\n";

    report += prename + ":NorthIndicator"  + " '" + QString::number(user->NorthIndicator) + "'\n";

    report += prename + ":UserString '" + QString(user->UserString) + "'\n";

    return report;

}// textPrintTelemetryOptions_t

/*!
 * Generate a string that describes the contents of a VideoOptions_t structure
 * \param prename is prepended to the name of the data field in the report
 * \param user is the structure to report
 * \return a string containing a report of the contents of user
 */
QString textPrintVideoOptions_t(QString prename, const VideoOptions_t* user)
{
    QString report;

    report += prename + ":Stabilized"  + " '" + QString::number(user->Stabilized) + "'\n";

    report += prename + ":ShowTrackBox"  + " '" + QString::number(user->ShowTrackBox) + "'\n";

    report += prename + ":ShowReticle"  + " '" + QString::number(user->ShowReticle) + "'\n";

    report += prename + ":RotateVideo"  + " '" + QString::number(user->RotateVideo) + "'\n";

    report += prename + ":DisableFovMatch"  + " '" + QString::number(user->DisableFovMatch) + "'\n";

    report += prename + ":ShowTelemetry"  + " '" + QString::number(user->ShowTelemetry) + "'\n";

    report += prename + ":HasTelemetryOptions"  + " '" + QString::number(user->HasTelemetryOptions) + "'\n";

    report += prename + ":ShowDetections"  + " '" + QString::number(user->ShowDetections) + "'\n";

    report += prename + ":DisableNtsc"  + " '" + QString::number(user->DisableNtsc) + "'\n";

    report += prename + ":EnablePip"  + " '" + QString::number(user->EnablePip) + "'\n";

    report += prename + ":ZoomTrackBox"  + " '" + QString::number(user->ZoomTrackBox) + "'\n";

    report += prename + ":MaxShift"  + " '" + QString::number(user->MaxShift, 'g', 16) + "'\n";

    report += textPrintTelemetryOptions_t(prename + ":TelemetryOptions", &user->TelemetryOptions);

    return report;

}// textPrintVideoOptions_t

/*!
 * Generate a string that describes the contents of a VideoOptions packet
 * \param prename is prepended to the name of the data field in the report
 * \param pkt is the data to print
 * \return a string describing the contents of pkt
 */
QString textPrintVideoOptionsPacket(const void* pkt)
{
    QString report;
    ...
    // Decode packet
    if(!decodeVideoOptionsPacketStructure(pkt, &user))
    {
        report = "VideoOptions packet failed to decode\n";
        return report;
    }

    // Print the packet size
    report += "VideoOptions packet size is " + QString::number(getOrionPublicPacketSize(pkt)) + "\n";

    report += textPrintVideoOptions_t("name", &user);

    return report;

}// textPrintVideoOptionsPacket
...
QString textPrintOrionNetworkByteSettings_t(QString prename, const OrionNetworkByteSettings_t* user)
{
    ...
    report += prename + ":MaxTcpClients"  + " '" + QString::number(user->MaxTcpClients) + "'\n";
...
QString textPrintOrionPayloadVersion_t(QString prename, const OrionPayloadVersion_t* user)
{
    ...
    report += prename + ":HwType"  + " '" + QString::number(user->HwType) + "'\n";

    report += prename + ":HwRevision"  + " '" + QString::number(user->HwRevision) + "'\n";
